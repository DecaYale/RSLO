...
def example_convert_to_torch(example, dtype=torch.float32,
    ...
    for k, v in example.items():
        ...
        #     example_torch[k] = calib
            ...
            example_torch[k] = [ [ torch.tensor(v[i][j], dtype=torch.float32, device=device) for j in range(len(v[i])) ] for i in range(len(v)) ]
...
# def proc_cov_pred(cov_param_pred):
#     #cov_param: Nx7 
#     #TODO
...
#     # return torch.cat([cov_param[:,:3], eigvec.reshape(-1,9) ], dim=-1 )
#     return torch.cat([cov_param[:,:3], (eigvec@eigval@eigvec.transpose(-1,-2)).reshape(-1,9) ], dim=-1 )



# def assign_gridconf_to_points( points, conf, conf_thresh=0.001, pc_range=[-70.4, -38.4, -3, 70.4, 38.4, 5], normalize_conf=True):
#     """
#     """

#     def grid_to_xyz(grid_coor, grid_size):
#         # grid_size = np.array(conf.shape[2:])[::-1]
#         grid_size=grid_size[::-1]#j,i,k , ::xyz
#         voxel_size = (np.array(pc_range[3:])-np.array(pc_range[:3]) )/grid_size  # x,y   #,z
...
#         return xv-voxel_size[0]/2,yv-voxel_size[1]/2, xv+voxel_size[0]/2,yv+voxel_size[1]/2
    

#     if normalize_conf:
#         conf=np.log(conf+1e-40)
#         conf=conf-np.percentile(conf[conf>-40],q=0) #conf.min()
#         conf[conf<0] = 0
#         conf/=conf.max()

#     grid_size = np.array(conf.shape)#[::-1]
#     highlight_mask=np.zeros_like(points[:,0])
#     for i in range(grid_size[1]):
#         for j in range(grid_size[2]):
#             # if conf[0,i,j]>conf_thresh:
#             if 1:
#                 xs, ys, xe, ye=grid_to_xyz( np.array([i,j]), grid_size )
#                 mask=(points[:,0]>=xs) & (points[:,0]<xe) & (points[:,1]>=ys) & (points[:,1]<ye)
#                 highlight_mask[mask] = conf[0,i,j]

#     # return highlight_mask>0.5 
#     return highlight_mask

# def pts2spherical_depth(self, pts, height, width, start_h = -0.46, end_h = 0.13):
#     pts_num = pts.shape[0]
#     delta_w = np.pi * 2 / width
#     delta_h = (end_h - start_h) / height
    
#     if 1:#self.lidar_type:
#         # index = np.full((height, width), -1, dtype=int)
#         index=np.frompyfunc(list, 0, 1)( np.empty((height, width), dtype=object) )               
        
#     else:
#         index = None

#     spherical_depth = np.zeros((height, width))
#     spherical_pts = np.zeros((height, width, 3))
#     x = pts[:, 0]
#     y = pts[:, 1]
#     z = pts[:, 2]
    
#     r = np.sqrt(np.square(x) + np.square(y) + np.square(z))
#     xy2 = np.sqrt(np.square(x) + np.square(y))
#     # alpha = np.arctan2(x, y)
#     alpha = np.arctan2(y, x)
#     beta = np.arctan2(z, xy2)
    
#     up_overflow_pts = 0
#     down_overflow_pts = 0
#     for idx in range(pts_num):
#         w = alpha[idx]
#         h = beta[idx]
#         if h >= end_h:
#             up_overflow_pts += 1
...
#         w = int((w + np.pi) / delta_w) % width
#         h = int((h - start_h) / delta_h)
#         if self.lidar_type:
#             index[height - h - 1, w] .append(idx)
#         if spherical_depth[h, w] == 0 or spherical_depth[h, w] > r[idx]:
#             spherical_pts[height - h - 1, w, :] = pts[idx, :3]
#             spherical_depth[height - h - 1, w] = r[idx]
        
#     if up_overflow_pts != 0:
#         print("up_overflow_pts = %d"%(up_overflow_pts))
#     if down_overflow_pts != 0:
#         print("down_overflow_pts = %d"%(down_overflow_pts))
    
#     return spherical_depth, spherical_pts, index
...
def evaluate(config_path,
    ...
    net = build_network(model_cfg, measure_time=measure_time, testing=True).cuda()#.to(device)
    ...
    for example in iter(eval_dataloader):
        if measure_time:
            ...
            t1 = time.time()
        example = example_convert_to_torch(example, float_dtype, get_rank(use_dist))
