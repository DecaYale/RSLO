...
def down_sample_points(points, down_sample_voxel_size ):
    ...
    return pcd
def create_data_info_hdf5(f, data_root, data_type='all',normal_data_root=None):
    """[summary]
        info structure:
        {
            0:[
    ...
    for seq in seqs:
        ...
        seq_grp.create_dataset("lidar_cross_normals", (seq_len,), dtype=dt, chunks=(3, ), maxshape=(None,) ) #Lx(Nx3)
        ...
        for idx in range(len(lidar_bin_paths)):
            ...
            #cross normals
            normal_path = os.path.join(normal_data_root,  info["lidar_bin_path"])
            cross_normals =  np.fromfile(
                    normal_path, dtype=np.float32,count=-1).reshape([-1, 7])[:, 4:]
            ...
            seq_grp["lidar_cross_normals"][idx] = cross_normals.reshape(-1) 
            ...
            print(lidar_path,  lidar_points.shape,lidar_normal.shape, lidar_normal[0],cross_normals[0], lidar_pcd, flush=True)
            # except:
...
def create_hdf5(data_root, hdf5_path, data_type='all',normal_data_root=None):
    f = h5py.File(hdf5_path, mode='w', rdcc_nbytes=1024**2*15) #15MB

    create_data_info_hdf5(f, data_root, data_type=data_type, normal_data_root=normal_data_root)
