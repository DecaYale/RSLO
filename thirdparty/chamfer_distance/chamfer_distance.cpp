...
int ChamferDistanceKernelLauncher(
    ...
    float* result2,
    int* result2_i);
int OneDirectionChamferDistanceKernelLauncher(
    const int b, const int n,
    const float* xyz,
    const int m,
    const float* xyz2,
...
int ChamferDistanceGradKernelLauncher(
    ...
    float* grad_xyz1,
    float* grad_xyz2);
int OneDirectionChamferDistanceGradKernelLauncher(
    const int b, const int n,
    const float* xyz1,
    const int m,
    const float* xyz2,
...
void one_direction_chamfer_distance_forward_cuda(
    const at::Tensor xyz1, 
    const at::Tensor xyz2, 
    const at::Tensor dist1, 
    // const at::Tensor dist2, 
...
{
    OneDirectionChamferDistanceKernelLauncher(xyz1.size(0), xyz1.size(1), xyz1.data<float>(),
                                            xyz2.size(1), xyz2.data<float>(),
                                            dist1.data<float>(), idx1.data<int>()
                                            // dist2.data<float>(), idx2.data<int>()
                                            );
...
void one_direction_chamfer_distance_backward_cuda(
    const at::Tensor xyz1,
    const at::Tensor xyz2, 
    at::Tensor gradxyz1, 
    at::Tensor gradxyz2, 
...
{
    OneDirectionChamferDistanceGradKernelLauncher(xyz1.size(0), xyz1.size(1), xyz1.data<float>(),
                                           xyz2.size(1), xyz2.data<float>(),
                                           graddist1.data<float>(), idx1.data<int>(),
    ...
    int* idx)
{
    for (int i = 0; i < b; i++) {
        for (int j = 0; j < n; j++) {
            ...
            dist[i*n+j] = best;
            idx[i*n+j] = besti; //the current best candidates in xyz2 for xyz1 
        }
    }
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("forward_cuda_one_direction", &one_direction_chamfer_distance_forward_cuda, "One direction ChamferDistance forward (CUDA)");
    ...
    m.def("backward_cuda_one_direction", &one_direction_chamfer_distance_backward_cuda, "One direction ChamferDistance backward (CUDA)");
}
